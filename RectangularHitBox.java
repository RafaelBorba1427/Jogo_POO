public class RectangularHitBox implements HitBox {
    ...
    public boolean intersects(CircularHitBox circle) {
        ...
        double closestX = valueBoundedInRange(localX, -halfWidth, halfWidth);

        double closestY = valueBoundedInRange(localY, -halfHeight, halfHeight);

        // Convert the closest point back to game coordinate system.
        Vector2D closestPoint = center
                .add(axisX.multiply(closestX))
                .add(axisY.multiply(closestY));

        // If the closest point is within the circle's radius,
        // they intersect.
        Vector2D difference = circle.getCenter().subtract(closestPoint);

        return difference.lengthSquared() <= circle.getRadius() * circle.getRadius();
    }

    // Returns the value if it is inside the range, else it returns the max or min
    // of the range
    ...
    private Projection projectOnto(Vector2D axis) {
         ...
         */
        double radius = Math.abs(normalizedAxis.dot(localX)) * halfWidth
                + Math.abs(normalizedAxis.dot(localY)) * halfHeight;

        double projectionCenter = center.dot(normalizedAxis);
