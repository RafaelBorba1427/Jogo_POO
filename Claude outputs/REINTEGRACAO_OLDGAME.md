...
- **Parte 2 — seções 10 a 15:** ícones animados do menu, volume nas opções, dois bugs corrigidos,
...
| `AnimationPlayer.java` | modificado | 1 linha: guarda de sprite nulo |
...
> ⚠️ **Colisão de IDs em `GameObject`** (não mexi, mas convém saber):
> `ID_BALL_3 = 16` é o mesmo valor de `ID_PERMANENT_FLOOR = 16`, e `ID_BALL_4 = 17` é o mesmo de
> `ID_PERMANENT_WALL = 17`. Para o `PointSystem` isso significa que uma bola com `ID_BALL_3`
> pontua 0, porque cai na entrada do chão. Não é um bug introduzido aqui, mas a hora de arrumar
> é antes de usar `ID_BALL_3` / `ID_BALL_4` de verdade.
