...
public class AABB {
    ...
    public AABB(RectangularHitBox OBB) {
        ...
        double aabbHalfWidth = Math.abs(OBB.getHalfWidth() * sine_cos.x) +
                Math.abs(OBB.getHalfHeight() * sine_cos.y);

        double aabbHalfHeight = Math.abs(OBB.getHalfWidth() * sine_cos.y) +
    ...
    public void alterSize(double x, double y) {
        min_pos.subtract(new Vector2D(x, y));
        min_pos.add(new Vector2D(x, y));
    }
